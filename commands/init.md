...
2. 使用当前宿主已有的正常 repo / 文件工具，直接创建或补齐最小工作区
3. 最小工作区至少包括：
   - `.feflow/project/`
   - `.feflow/items/`
...
4. 若存在轻量扫描能力，可补入保守默认值；若暂时无法确认，则标记为 `unknown`，不阻塞初始化
5. 若宿主提供正常文件工具，必须在**当前这一回合**真实完成至少一次目录创建、文件生成、或缺失项补全；不能只描述“接下来会做什么”
6. 初始化完成后输出最小状态摘要和推荐的下一步
