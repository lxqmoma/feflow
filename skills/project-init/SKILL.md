...
- 用户执行 `/feflow-init` 或明确要求初始化 feflow
...
自动扫描完成后，向用户提出 5-6 个机器无法自动判断的关键问题。以交互式问答方式逐个确认。
...
2. **分支策略**
   > 日常开发使用什么分支模型？（举例：feature → develop → release → main / feature → main / trunk-based / 其他）

3. **工作项绑定要求**
   > 每次提交是否需要关联工作项 ID？ID 格式是什么？（举例：JIRA-1234 / #123 / 不需要 / 其他）

4. **高风险模块**
   > 哪些目录或模块属于高风险区域，修改时需要额外谨慎？（举例：支付模块、权限系统、数据库迁移、无）

5. **AI 编码限制**
   > 有哪些文件或目录是 AI 不应该直接修改的？（举例：安全相关配置、生产环境脚本、第三方 SDK 封装、无特殊限制）

6. **审批门禁**（如检测到 CI/CD 配置则追加）
   > 合并到主分支前需要哪些审批？（举例：至少 1 人 review / CI 全绿 / 无要求）
