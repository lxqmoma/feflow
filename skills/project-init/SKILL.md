...
`workspace_shape` / `release_detection` / `branch_strategy` / `workflow_binding` / `item_id_format` / `backfill` / `testing_policy` / `approval_gates` / `high_risk_modules` / `ai_restrictions` / `initialized_at` / `initialized_by`
