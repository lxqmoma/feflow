---
name: dashboard
description: Item 状态全景视图。展示所有工作项的状态分布、进度概览和风险提示。
---

# dashboard：Item 状态全景视图

展示所有工作项的状态分布、进度概览、风险提示和依赖阻塞情况。
...
总数、各状态数量、平均停留时间（implementing / testing 阶段）。
...
读取 `.feflow/memory/` 统计条目状态（active / stale）。stale 定义：超过 30 天未更新。stale 占比超 30% 时提示执行 `memory-decay`。
